...
class tables{
	...
	public static function tableFilter(){
		...
		foreach($tableData as $data){
			...
			$sedeCell = new html_table_cell($data->facultad);
			...
			$editUrl = new moodle_url("/local/omegasync/crear.php", array('action'=>'edit_form','pAcademicoId'=>$array->omegasync_id));
			$editButton = $OUTPUT->action_icon($editUrl, $editIcon);
			
			$table->data[]= array($data->periodo_academico, 'Prueba!',$sedeCell,'Santiago-<br>Penalolen',date('l, F d Y', $data->fecha_creacion).' '.date('H:i',$data->fecha_creacion),
								  date('l, F d Y', $data->fecha_creacion).' '.date('H:i',$data->fecha_creacion),date('l, F d Y', $data->fecha_creacion).' '.date('H:i',$data->fecha_creacion),
